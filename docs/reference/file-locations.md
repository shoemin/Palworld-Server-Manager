...
Application binaries (the portable build today, or the not-yet-released Setup.exe installer) live wherever you extracted/installed them — never inside the path above. The installer's own default install location, verified with a real local install/uninstall cycle, is `%LocalAppData%\ShoeMin.PalworldServerManager\` — a different package identifier deliberately chosen so it can never collide with the persistent-data root. See [Installation](../getting-started/installation.md) for why that separation is a hard requirement, not an accident.
