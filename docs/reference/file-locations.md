...
Application binaries (either the installed Setup.exe build or the portable build) live wherever you installed/extracted them — never inside the path above. The installer's own default install location, verified with a real local install/uninstall cycle, is `%LocalAppData%\ShoeMin.PalworldServerManager\` — a different package identifier deliberately chosen so it can never collide with the persistent-data root. See [Installation](../getting-started/installation.md) for why that separation is a hard requirement, not an accident.
