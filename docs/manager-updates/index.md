...
!!! success "Field status: live self-update validated with Palworld running"
    Palworld Server Manager can **check for**, **download**, and **install** its own updates from a genuine Setup.exe install, using [Velopack](https://velopack.io) — including restarting itself while a Palworld server keeps running. This has now been field-tested end to end: a real installed `v0.4.0-alpha.1` checked for, downloaded, and installed the published `v0.4.0-alpha.2` update while a real Palworld dedicated server was running throughout. The Palworld Shipping process kept the **exact same process ID and start time** before and after the Manager was replaced — proof it was never restarted — and the new Manager correctly reattached to it. No REST save/shutdown or SteamCMD update was triggered by the Manager update itself; LAN and Dashboard/REST polling resumed automatically, and a **Safe Stop** performed afterward on the reattached server succeeded normally. One related scenario is not yet field-tested: a genuinely **in-flight Manager-owned operation actively blocking Install and Restart** (automated tests cover this; the field attempt used Backup, which correctly refuses to run against a running server before that blocker is ever reached — a sustained LAN transfer is planned for the next update cycle). Nothing on this page is available to Portable-mode users; see [Installation](../getting-started/installation.md).
