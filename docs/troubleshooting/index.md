...
**Check for Updates says it's up to date, but I expected a newer version.** A few things to check: confirm the [execution mode](../manager-updates/index.md#execution-modes) is actually **Installed**, not Portable or Development (those can't check at all). Confirm which [channel](../manager-updates/update-channels.md) is selected — **Prerelease** maps to the `win-beta` Velopack channel, **Stable** maps to `win`. Prerelease releases exist today, so a genuine "up to date" result there means exactly that. Stable currently has no published release at all, so checking on Stable shows an error rather than "up to date" — see [Current release state](../manager-updates/update-channels.md), that's expected given today's release state. The Manager also never offers the version you're currently running as an "update," and it never automatically offers to downgrade to an older version on either channel — see [No automatic downgrade](../manager-updates/update-channels.md#no-automatic-downgrade).

**Check for Updates on the Stable channel shows an error.** Expected right now — there is no Stable/`win` release published yet, so there's no release feed to query at all, and that's reported the same way a network/GitHub failure would be. See [Current release state](../manager-updates/update-channels.md). If you're testing this build, switch to the Prerelease channel instead.
