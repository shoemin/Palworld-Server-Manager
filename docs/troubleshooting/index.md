...
**Check for Updates says it's up to date, but I expected a newer version.** A few things to check: confirm the [execution mode](../manager-updates/index.md#execution-modes) is actually **Installed**, not Portable or Development (those can't check at all). Confirm which [channel](../manager-updates/update-channels.md) is selected — **Prerelease** maps to the `win-beta` Velopack channel, **Stable** maps to `win`. If the channel you're on genuinely has no newer release than what you're running, "up to date" is correct. If instead that channel currently has *zero* published releases at all, checking shows an error rather than "up to date" — see [If a channel has no published releases yet](../manager-updates/update-channels.md). The Manager also never offers the version you're currently running as an "update," and it never automatically offers to downgrade to an older version on either channel — see [No automatic downgrade](../manager-updates/update-channels.md#no-automatic-downgrade).

**Check for Updates on a channel shows an error instead of "up to date."** This happens when that specific channel has zero published releases — there's no release feed at all for the Manager to query, and that's reported the same way a network/GitHub failure would be. See [If a channel has no published releases yet](../manager-updates/update-channels.md). Try switching to the other channel, or confirm you selected the one you meant to.
