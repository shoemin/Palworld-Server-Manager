...
- The full nupkg (and delta nupkg, if one was generated) — discovered from disk rather than string-built, because the Windows-default-channel naming rule drops the channel suffix entirely for `win` (`<PackId>-<version>-full.nupkg`) but keeps it for every other channel (`<PackId>-<version>-<channel>-full.nupkg`).
- `releases.<channel>.json` — always.
- A legacy file named exactly `RELEASES` (no channel suffix) — **only** when publishing to the default `win` channel. `vpk pack` locally writes a channel-suffixed `RELEASES-<channel>` file for every channel for its own bookkeeping, but Velopack's GitHub uploader never uploads that channel-suffixed file for any channel — only the unsuffixed `RELEASES`, and only for `win`.
