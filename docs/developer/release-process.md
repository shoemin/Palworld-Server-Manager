...
5. Best-effort downloads the previous release for the same channel (`vpk download github`) into the same output directory, so `vpk pack`'s default delta mode can generate a delta package against it automatically. Pinned `vpk` 1.2.0's `--pre` flag defaults to `false` ("Get latest pre-release instead of stable"), and this project's real releases on the `win-beta` (Prerelease) channel are always published as GitHub prereleases — so the download passes `--pre` whenever the channel being built is a prerelease, and omits it for the `win` (Stable) channel, which must only ever consider non-prerelease releases. Without this, `vpk download github` would report zero releases for `win-beta` even with a real previous release published (verified directly against this repository). This step is allowed to find nothing (a fresh channel, or the very first release) without failing the workflow — the retrieved previous full nupkg is only a local delta-generation input and is never itself republished as part of the new release.
...
9. Uploads the packaging output as a workflow-run artifact and the build transcript as a separate artifact.
10. Publishes the release via `vpk upload github`.
11. Only after publication succeeds: downloads every intended public asset back from the real GitHub Release, generates `SHA256SUMS.txt` from those downloaded bytes (not the local packaging-stage copies), uploads it, then re-downloads the published manifest and independently re-verifies it against the same downloaded bytes — see [Public checksum manifest scope](#public-checksum-manifest-scope) below.
...
`assets.<channel>.json` is local-only build bookkeeping (`BuildAssets.Write`) and is never uploaded for any channel, so it's deliberately excluded rather than filtered after the fact.

**The manifest's *bytes*, however, come only from the actual published GitHub Release — never from the local packaging directory.** After `vpk upload github` completes, the workflow re-queries the real Release's asset list (requiring it to contain exactly the intended set, plus an optional pre-existing `SHA256SUMS.txt` tolerated only so a `workflow_dispatch` recovery rerun can replace it with `--clobber`), downloads every intended asset back by exact filename, and generates `SHA256SUMS.txt` by hashing those downloaded copies. It then uploads that manifest and, as a final proof step, downloads the *published* `SHA256SUMS.txt` back and independently re-verifies every entry — count, filenames, no duplicates, no self-entry, and every hash — against the same downloaded bytes. The workflow fails closed at every one of these checks rather than assuming a prior step's success implies the next one is correct.

This two-phase design (names decided before publication, bytes hashed only after) exists because **v0.4.0-alpha.2 proved the two are not interchangeable**: `vpk upload github`'s `GitHubRepository.UploadMissingAssetsAsync` doesn't upload the local on-disk `releases.<channel>.json` verbatim — it regenerates that one file's JSON content fresh in-memory from only the assets packed in the current run, before uploading it. The local copy legitimately differs whenever a previous release's full nupkg was present in the same output directory (i.e. on every release after a channel's first, since that file is deliberately left there for delta diffing) — its feed lists that leftover previous-version entry too, while the regenerated published version correctly does not. alpha.2's `SHA256SUMS.txt` was generated the old way (hashing the local file *before* publication) and its `releases.win-beta.json` entry does not match the bytes actually published — confirmed via a real published-asset audit, not a hypothetical. The public Velopack update feed itself was never affected by this; only the checksum manifest's description of that one file was wrong. This is a release-pipeline checksum-manifest defect, not a Manager runtime or update-feed defect. alpha.2 is immutable and was not corrected retroactively; this is fixed prospectively starting with the next release.

v0.4.0-alpha.1's published `SHA256SUMS.txt` had an earlier, unrelated version of this same class of problem: it was generated by a blind-directory-listing approach and lists `assets.win-beta.json` and `RELEASES-win-beta` as if they were downloadable — they were never actually uploaded (expected Velopack behavior, not a defect in what got published), but the checksum manifest describing them was misleading. alpha.1 is likewise immutable and was not corrected retroactively.
