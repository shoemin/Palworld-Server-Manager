...
public static class WindowsHostComposition
{
    ...
    internal static PeerRotationReceiptRpcClient CreatePeerRotationReceiptClient(PeerSecurityRpcRuntime rpc, X509Certificate2 certificate)
        => new(rpc, new WindowsPeerHttpTransportFactory(certificate));
    internal static PeerCurrentCredentialRpcClient CreatePeerCurrentCredentialClient(PeerSecurityRpcRuntime rpc, X509Certificate2 certificate)
        => new(rpc, new WindowsPeerHttpTransportFactory(certificate));
    internal static PeerUnpairConnectionFactory CreatePeerUnpairConnectionFactory(PeerSecurityRpcRuntime rpc,X509Certificate2 certificate)
        =>new(rpc,new WindowsPeerHttpTransportFactory(certificate));
