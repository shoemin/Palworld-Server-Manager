...
internal static class RotationStagingTests
{
    ...
    public static async Task ReceiverOrderingReplayAndRollback()
    {
        ...
        var results = await Task.WhenAll(Enumerable.Range(0, 12).Select(_ => Task.Run(() => f.Repository.StagePeerRotation(first, Old))));
        ...
        Reject<AuthenticationException>(() => f.Repository.StagePeerRotation(Proposal(f, 2) with { OldFingerprint = Next, NewFingerprint = Later }, Next));
        f.Time.Now += TimeSpan.FromMinutes(2);
        Check(new PeerTrustRepository(f.Database, f.HostId, f.Time).StagePeerRotation(first, Old).ExpiresUtc == deadline);
        Reject<AuthenticationException>(() => f.Repository.StagePeerRotation(first with { NewFingerprint = Later }, Old));
        Reject<AuthenticationException>(() => f.Repository.StagePeerRotation(first with { Sequence = 2 }, Old));
        Reject<AuthenticationException>(() => f.Repository.StagePeerRotation(Proposal(f), Old));
        ...
        Reject<SqliteException>(() => f.Repository.StagePeerRotation(newer, Old)); Check(f.Count("PeerRotationProposals") == 1 && f.Repository.Read(f.PeerId)!.PendingRotationId == first.RotationId);
        f.Execute("DROP TRIGGER FailStage;");
        Check(f.Repository.StagePeerRotation(newer, Old).Disposition == PeerRotationStagingDisposition.Staged);
        Reject<AuthenticationException>(() => f.Repository.StagePeerRotation(first, Old));
    ...
    {
        using var f = new PeerTrustTests.Fixture(); Active(f); var first = Proposal(f); f.Repository.StagePeerRotation(first, Old);
        ...
        Reject<SqliteException>(() => f.Repository.StagePeerRotation(newer, Old));
        ...
        var blocked = f.Repository.StagePeerRotation(newer, Old);
        ...
        Check(f.Repository.Read(f.PeerId)!.PendingReconfirmationRequired && f.Count("PeerRotationProposals") == 1);
        Check(f.Repository.StagePeerRotation(first, Old).Disposition == PeerRotationStagingDisposition.ReconfirmationRequired);
        f.Repository.ObserveActivePeerCredential(f.PeerId, Next);
        var afterPromotion = newer with { OldFingerprint = Next, Sequence = 1 };
        blocked = f.Repository.StagePeerRotation(afterPromotion, Next);
        ...
        Check(f.Repository.StagePeerRotation(afterPromotion, Next).Disposition == PeerRotationStagingDisposition.Staged); // New current-key epoch.
        Reject<AuthenticationException>(() => f.Repository.StagePeerRotation(first, Old));
    ...
    public static Task ClosedStatesAndIdentity()
    {
        ...
        f.Repository.RecordVerifiedBinding(f.PeerId, Old, Local);
        Reject<AuthenticationException>(() => f.Repository.StagePeerRotation(proposal, Old));
        ...
        foreach (var invalid in new[] { proposal with { Sequence = 0 }, proposal with { Sequence = -1 }, proposal with { HostId = f.HostId }, proposal with { HostId = Guid.NewGuid() }, proposal with { NewFingerprint = Old } })
            Reject<AuthenticationException>(() => f.Repository.StagePeerRotation(invalid, Old));
        Reject<AuthenticationException>(() => f.Repository.StagePeerRotation(proposal, Next));
    ...
    public static Task UpgradeDoesNotInventOrdering()
    {
        ...
        Reject<InvalidDataException>(() => f.Repository.StagePeerRotation(Proposal(f), Old));
        ...
        Check(f.Repository.StagePeerRotation(Proposal(f) with { OldFingerprint = Next, NewFingerprint = Later }, Next).Disposition == PeerRotationStagingDisposition.Staged);
